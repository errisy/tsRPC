...
namespace ts {
     export let programTime = 0;
     export let emitTime = 0;
     export let ioReadTime = 0;
     export let ioWriteTime = 0;
    ...
    export function createCompilerHost(options: CompilerOptions, setParentNodes?: boolean): CompilerHost {
        tscDebugger.enter('createCompilerHost');
        ...
        function getSourceFile(fileName: string, languageVersion: ScriptTarget, onError?: (message: string) => void): SourceFile {
            tscDebugger.enter('getSourceFile');
            let text: string;
            try {
            ...
            tscDebugger.exit('getSourceFile');
        ...
        tscDebugger.exit('createCompilerHost');
        return {
            getSourceFile,
    ...
    export function createProgram(rootNames: string[], options: CompilerOptions, host?: CompilerHost, oldProgram?: Program): Program {
        tscDebugger.enter('createProgram');
        let program: Program;
        ...
        tscDebugger.comment('Before Resolving');
        ...
        tscDebugger.comment('After Resolving');
        const filesByName = createFileMap<SourceFile>();
        tscDebugger.comment('After createFileMap');
        // stores 'filename -> file association' ignoring case
        // used to track cases when two file names differ only in casing
        const filesByNameIgnoreCase = host.useCaseSensitiveFileNames() ? createFileMap<SourceFile>(fileName => fileName.toLowerCase()) : undefined;
        tscDebugger.comment('After filesByNameIgnoreCase');
        if (!tryReuseStructureFromOldProgram()) {
            ...
            if (!skipDefaultLib) {
                ...
                if (!options.lib) {
                    ...
                    tscDebugger.comment('After processRootFile !options.lib');
                }
                else {
        ...
        tscDebugger.comment('Before program created');
        program = {
        ...
        tscDebugger.comment('After program created');
        verifyCompilerOptions();

        programTime += new Date().getTime() - start;
        tscDebugger.exit('createProgram');
        return program;
        ...
        function processSourceFile(fileName: string, isDefaultLib: boolean, isReference: boolean, refFile?: SourceFile, refPos?: number, refEnd?: number) {
            tscDebugger.enter('processSourceFile', fileName);
            ...
            tscDebugger.exit('processSourceFile');
        }
