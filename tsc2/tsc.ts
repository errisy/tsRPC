...
namespace ts {
    ...
    export function executeCommandLine(args: string[]): void {
        tscDebugger.enter('executeCommandLine');
        ...
        function parseConfigFile(): ParsedCommandLine {
            tscDebugger.enter('parseConfigFile', configFileName);
            if (!cachedConfigFileText) {
                try {
                    ...
                    tscDebugger.exit('parseConfigFile');
                    return;
                }
            }
            if (!cachedConfigFileText) {
        ...
        function performCompilation() {
            tscDebugger.enter('performCompilation');
            if (!cachedProgram) {
                if (configFileName) {
            ...
            if (!isWatchSet(compilerOptions)) {
                tscDebugger.exit('performCompilation');
    ...
    function compile(fileNames: string[], compilerOptions: CompilerOptions, compilerHost: CompilerHost) {
        tscDebugger.enter('compile', fileNames);
        ioReadTime = 0;
        ioWriteTime = 0;
        programTime = 0;
        bindTime = 0;
        ...
        function compileProgram(): ExitStatus {
            tscDebugger.enter('compileProgram');
            ...
            tscDebugger.comment('After emit');
            ...
            if (emitOutput.emitSkipped && diagnostics.length > 0) {
                ...
                tscDebugger.exit('compileProgram');
...
tscDebugger.enter('TSC Entry Point');
ts.executeCommandLine(ts.sys.args);
tscDebugger.exit('TSC Entry Point');